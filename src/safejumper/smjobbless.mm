...
bool installPrivilegedHelperTool()
{
    ...
    CFDictionaryRef installedHelperJobData = SMJobCopyDictionary(kSMDomainSystemLaunchd, CFSTR(kSAFEJUMPER_HELPER_LABEL));
    ...
    if (installedHelperJobData)
    {
        ...
        CFStringRef helperToolPath = CFStringCreateWithFormat(kCFAllocatorDefault, NULL, CFSTR("Contents/Library/LaunchServices/%@"), CFSTR(kSAFEJUMPER_HELPER_LABEL));
        ...
        if ( currentVersion == installedVersion )
        {
            ...
            CFStringRef reqStr = CFStringCreateWithFormat(kCFAllocatorDefault, NULL, CFSTR("identifier %@ and certificate leaf[subject.CN] = \"%@\""),
                                                          CFSTR(kSAFEJUMPER_HELPER_LABEL), CFSTR(kSigningCertCommonName));
    ...
    if (needToInstall)
    {
        if (!blessHelperWithLabel(CFSTR(kSAFEJUMPER_HELPER_LABEL), &error))
        {
            if (error) {
