...
printf("PLUGIN: before FwpmEngineOpen0()\n");
            dwFwAPiRetCode =  ::FwpmEngineOpen0( NULL,
                                                 RPC_C_AUTHN_WINNT,
                                                 NULL,
                                                 &session,
                                                 &m_hEngineHandle );
			printf("PLUGIN: FwpmEngineOpen0 returned %X\n", dwFwAPiRetCode);
        }
        else
        {
...
printf("PLUGIN: before UuidCreate()\n");
            rpcStatus = ::UuidCreate(&SubLayer.subLayerKey);
printf("PLUGIN: UuidCreate returned %X\n", rpcStatus);
            if( NO_ERROR == rpcStatus )
            {
                // Save GUID.
printf("PLUGIN: before CopyMemory()\n");
...
printf("PLUGIN: before FwpmSubLayerAdd0()\n");
                dwFwAPiRetCode = ::FwpmSubLayerAdd0( m_hEngineHandle,
                                                     &SubLayer,
                                                     NULL );
				printf("PLUGIN: FwpmSubLayerAdd0 returned %X, %d\n", dwFwAPiRetCode, dwFwAPiRetCode);
            }
printf("PLUGIN: end of block\n");
        }
        else
        {
