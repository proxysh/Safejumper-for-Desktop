...
SUBDIRS += \
    ...
    RESOURCES_INST_DIR = $$shell_quote($$OUT_PWD/shieldtra/$${APPNAME}/Contents/Resources/)

    # Commands to organize the bundle app
    organizer.commands += $(MKDIR) $$shell_quote($$OUT_PWD/shieldtra/$${APPNAME}/Contents/Library/LaunchServices);
    organizer.commands += $(COPY) $$shell_quote($$OUT_PWD/$${HELPERAPP}) $$shell_quote($$OUT_PWD/shieldtra/$${APPNAME}/Contents/Library/LaunchServices);
    organizer.commands += $(MKDIR) $$shell_quote($$OUT_PWD/shieldtra/$${APPNAME}/Contents/Resources);
    organizer.commands += $(COPY) $$PWD/service/$${HELPERAPP_INFO} $$shell_quote($$OUT_PWD/shieldtra/$${APPNAME}/Contents/Resources);
    organizer.commands += $(COPY) $$PWD/service/$${HELPER_APP_LAUNCHD_INFO} $$shell_quote($$OUT_PWD/shieldtra/$${APPNAME}/Contents/Resources);
    ...
    codesigner.commands += macdeployqt $$shell_quote($${OUT_PWD}/shieldtra/$${APPNAME}) -always-overwrite -codesign=$${CERTSHA1};

    # set the modification and access times of files
    codesigner.commands += touch -c $$shell_quote($${OUT_PWD}/shieldtra/$${APPNAME});
